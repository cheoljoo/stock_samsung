...
def get_stock_data_with_diff_and_dividends(ticker1, ticker2, start_date, end_date, external_dividends=None):
    ...
    try:
        ...
        def calculate_rolling_quantile(series, window_size, quantile):
            """슬라이딩 윈도우로 분위수 계산"""
            result = []
            for i in range(len(series)):
                if i < window_size:
                    # 데이터가 윈도우 크기보다 적으면 처음부터 현재까지의 데이터 사용
                    window_data = series.iloc[:i+1]
                else:
                    # 지정된 기간 데이터 사용 (현재 날짜 포함 이전 기간)
                    window_data = series.iloc[i-window_size+1:i+1]
                
                if len(window_data) > 0:
                    result.append(window_data.quantile(quantile))
                else:
                    result.append(0)
            return result
        ...
        for window_name, window_days in window_configs.items():
            ...
            combined_df[f'Price_Diff_Ratio_25th_Percentile_{window_name}'] = calculate_rolling_quantile(
                combined_df['Price_Diff_Ratio'], window_days, 0.25
            )
            combined_df[f'Price_Diff_Ratio_75th_Percentile_{window_name}'] = calculate_rolling_quantile(
                combined_df['Price_Diff_Ratio'], window_days, 0.75
            )

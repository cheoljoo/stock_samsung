...
def run_backtest(json_file_path, initial_stock_type, initial_shares, start_date_str):
    ...
    try:
        ...
        print("\n=== 기본 전략: 2년 슬라이딩 윈도우 사분위 기반 매매 전략 ===")
        print("- 가격차이비율 < 2년 슬라이딩 25% 분위: 삼성전자 매수 (상대적 저평가)")
        print("- 가격차이비율 > 2년 슬라이딩 75% 분위: 삼성전자(우) 매수 (상대적 저평가)")
        print("- 2년 윈도우: 해당 날짜 이전 730일 데이터로 사분위 계산\n")

        strategy_results['기본전략'] = run_single_strategy(df_backtest, initial_stock_type, initial_shares, initial_value, False)
        
        print("\n" + "="*60)
        print("=== 반대 전략: 2년 슬라이딩 윈도우 사분위 기반 매매 전략 ===")
        print("- 가격차이비율 < 2년 슬라이딩 25% 분위: 삼성전자(우) 매수")
        print("- 가격차이비율 > 2년 슬라이딩 75% 분위: 삼성전자 매수")
        print("- 2년 윈도우: 해당 날짜 이전 730일 데이터로 사분위 계산\n")

        strategy_results['반대전략'] = run_single_strategy(df_backtest, initial_stock_type, initial_shares, initial_value, True)

def run_single_strategy(df_backtest, initial_stock_type, initial_shares, initial_value, reverse_strategy=False):
    """
    ...
    trading_log = [] # 매매 기록 저장
        ...
        if not trades_only.empty:
            ...
            print(f"삼성전자 -> 삼성전자(우): {samsung_to_pref}회 (2년 윈도우 75% 분위 초과 시)")
            print(f"삼성전자(우) -> 삼성전자: {pref_to_samsung}회 (2년 윈도우 25% 분위 미만 시)")
        ...
        plt.plot(monthly_strategy_df.index, monthly_strategy_df['Value'], label='2년 슬라이딩 윈도우 사분위 전략', marker='o', markersize=4)
        plt.plot(monthly_buy_hold_df.index, monthly_buy_hold_df['Value'], label='삼성전자 Buy & Hold', marker='x', markersize=4)

        plt.title('월별 포트폴리오 평가 금액 비교 (2년 슬라이딩 윈도우 사분위 전략 vs Buy&Hold)', fontsize=14)
        plt.xlabel('날짜')
        plt.ylabel('평가 금액 (원)')
        plt.grid(True)
        plt.legend()
        ...
        fig.suptitle('시간에 따른 주식 보유 현황 (2년 슬라이딩 윈도우 사분위 전략)', fontsize=16)
        ...
        ax.set_title('시간에 따른 주식 보유 현황 및 매매 시점 (2년 슬라이딩 윈도우 사분위 전략)')
        ax.set_xlabel('날짜')

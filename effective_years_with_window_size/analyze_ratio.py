    ...
    try:
        ...
        if len(available_columns) >= 2:
            ...
            if 'Price_Diff_Ratio' in df.columns:
                print("두 번째 그래프: 가격 차이 비율 및 2년 슬라이딩 윈도우 사분위수")
                plt.figure(figsize=(15, 8))
                plt.plot(df.index, df['Price_Diff_Ratio'], label='가격 차이 비율', color='purple', linewidth=1.5)
                plt.title('가격 차이 비율 (%) - 2년 슬라이딩 윈도우 사분위수 포함', fontsize=16)
                plt.ylabel('비율 (%)')
                plt.xlabel('날짜')
...
def generate_timeseries_plots_for_all_periods():
    """
    모든 기간과 윈도우 사이즈에 대해 price_diff_ratio 시계열 그래프를 생성합니다.
    """
    periods = ['3년', '5년', '10년', '20년', '30년']
    window_sizes = ['2년', '3년', '5년']
    
    print("=== 기간별 및 윈도우 사이즈별 시계열 그래프 생성 ===\n")
    ...
    for period in periods:
        ...
        try:
            ...
            for window_size in window_sizes:
                ...
                plt.title(f'가격 차이 비율 ({period}, {window_size} 윈도우)', fontsize=16)
                plt.ylabel('비율 (%)')
                plt.xlabel('날짜')
                plt.grid(True, alpha=0.3)
                plt.xticks(rotation=45)
                ...
                # 파일명 생성
                filename = f'price_diff_ratio_timeseries_{period}_{window_size}.png'
...
if __name__ == "__main__":
    # 모든 기간 및 윈도우 사이즈별 시계열 그래프 생성
    generate_timeseries_plots_for_all_periods()

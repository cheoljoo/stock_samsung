    ...
    try:
        ...
        for i, (date, row) in enumerate(df_backtest.iterrows()):
            ...
            if i == 0:
                ...
                # 매매 조건 확인 (반대 전략)
                if current_ratio < q25 and current_stock_type != '삼성전자(우)':
                    # 삼성전자 -> 삼성전자(우)
                ...
                elif current_ratio > q75 and current_stock_type != '삼성전자':
                    # 삼성전자(우) -> 삼성전자
        ...
        if not trades_only.empty:
            ...
            print(f"삼성전자 -> 삼성전자(우): {samsung_to_pref}회 (25% 이하에서 매수)")
            print(f"삼성전자(우) -> 삼성전자: {pref_to_samsung}회 (75% 이상에서 매수)")
